...
AdolfoClassifierParser::AdolfoClassifierParser() {
    diffEvo = new DifferencialEvolution();
    evoStrat = new EvolutionStrategie();
}

string AdolfoClassifierParser::nameParser() {
    return "AdolfoClassifierParser";
}

void AdolfoClassifierParser::Optimize(Subject* s){
...
double AdolfoClassifierParser::Evaluate(Subject* s) {
    //return  1;
    double totalFit = 0, fit, r;

    double familiar, autorizado;
    double tFamiliar = 0;
    double tLat = 0;
...
//            familiar = dataset[j][data->variables + arvore];
//            autorizado = r;
//
//            if(familiar && autorizado){
//                tFamiliar++;
////                cout << "autorizado familiar" << endl;
//            }
//            else if(!familiar && autorizado){
//                tLat++;
////                cout << "autorizado outro" << endl;
//            }
        }

//        cout << tFamiliar << " " << tLat/tamDataset << " " << abs(0.6 - tLat/tamDataset) << endl;
//        cin.get();
...
double AdolfoClassifierParser::Operate(int opType, int opValue, double a, double b, double c) {
    double r;
...
double AdolfoClassifierParser::AuxEvaluate(Subject* s, int model, double* dat) {
